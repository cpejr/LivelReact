    ...
    <div style={{ position: "relative" }}>
      ...
      {parts.map((part, index) => {
        return (
          <div key={index}>
            {
              //No objeto de cada parte, existe um atributo (boolean) se esta selecionado ou não
              //Se estiver selecionado, aparece a imagem vermelha (ou seja, a imagem -dor)
              //Caso contrário, a imagem normal
              part.selected ? (
                <img
                  src={`/images/body/${part.part_name}-dor.png`}
                  alt={part.part_name}
                  style={partStyle(part.top, part.left)}
                  onClick={() => handleSelect(index)}
                />
              ) : (
                <img
                  src={`/images/body/${part.part_name}.png`}
                  alt={part.part_name}
                  style={partStyle(part.top, part.left)}
                  onClick={() => handleSelect(index)}
                />
